...
- Additive Vim-style task-list navigation is also supported in normal/list mode: `j`/`k` move down/up, `g`/`G` jump to the top/bottom, `l` expands the selected task, `h` collapses an expanded task, and pressing `d` again while delete confirmation is open works like a `dd`-style delete confirm.
