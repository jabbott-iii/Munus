...
func (m *ListModel) applyImportFromTransfer() (tea.Model, tea.Cmd) {
	...
	if len(res.SkippedIDs) > 0 {
		status += fmt.Sprintf(" • skipped ids=%s", strings.Join(res.SkippedIDs, ", "))
	}
	if res.BackupPath != "" {
...
func (m *ListModel) renderTransferOverlay(baseView string) string {
	...
	if m.transfer.action == transferActionExport {
		...
		if m.transfer.stage == transferStageConfirm && m.transfer.plan != nil {
			...
			dialog.WriteString("\n")
			_, err = fmt.Fprintf(&dialog, "Skip existing IDs: %s\n", yesNoLabel(m.transfer.skipExisting))
			if err != nil {
				return ""
			}
			...
			_, err = fmt.Fprintf(&dialog, "  Skip: %d\n", m.transfer.plan.ToSkip)
			if err != nil {
				return ""
			}
			...
			if len(m.transfer.plan.ConflictIDs) > 0 {
				_, err = fmt.Fprintf(&dialog, "  Conflict IDs: %s\n", strings.Join(m.transfer.plan.ConflictIDs, ", "))
				if err != nil {
					return ""
				}
			}
			...
			_, err = fmt.Fprintf(&dialog, "Skip existing IDs: %s (press alt+o to toggle)\n", yesNoLabel(m.transfer.skipExisting))
			if err != nil {
				return ""
			}
